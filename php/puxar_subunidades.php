...
if ($query == true) {
		...
		$resultado .="<li>
					  <a href=\"#\" id=\"id_".$dados['id']."\" onclick=\"recebeDados(".$dados['id'].")\" class=\"item-link item-content\">
						<div style=\"text-transform:uppercase\" class=\"item-inner item-cell\">
							<div class=\"item-row\">
							  <div class=\"item-cell\" id=\"subunidade_".$dados['id']."\">".$dados['subunidade']."</div>
							</div>
						</div>
					  </a>
					</li>
	...
	app.dialog.confirm('Tem certeza que quer deletar <b>".$dados['subunidade']."</b>?','<b>CONFIRMAÇÃO</b>', function () {
				...
				app.dialog.preloader('Deletando...');
						...
						url: 'https://interappctive.com/admciabm/deletar_subunidade.php',  
		...
		$resultado.="<script>
					function recebeDados(id){
						var unidade=$(\"#subunidade_\"+id).html();
						
						$(\"#cadastro_subobm\").val(unidade);
						$(\"#CampoID\").val(id);
						
						$(\"#BotaoSalvar\").addClass('display-none');
						$(\"#BotaoEditar_subobm\").removeClass('display-none');
						$(\"#cad_subunidade\").removeClass('display-none');
		...
		$resultado .="<li>
					  <div class=\"item-content\">
						<div class=\"item-inner\">
						  <div class=\"item-title\"><b>Nenhuma Subunidade Encontrada</b></div>
