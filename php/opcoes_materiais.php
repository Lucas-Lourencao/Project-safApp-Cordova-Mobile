...
$puxar="SELECT * FROM `cadastro_materiais` WHERE subunidade='$subunidade'";
...
if ($query == true) {
	...
	if ($quantos>0){
		...
		$resultado.="<option value='".$dados['material']."'>".$dados['material']."</option>";
		}
			
		
	//SE A QUANTIDADE DE REGISTROS É 0	
	}else{ 
		$resultado.="<option value=''>Material não encontrado!";
	}
